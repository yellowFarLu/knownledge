...
![img](https://ws2.sinaimg.cn/large/006tNbRwgy1fxcc9ctmjlg30e801s74s.gif)
...
接下来我们使用**位操作将字符串设置为 hello**(不是直接使用 set 指令)，首先我们需要得到 hello 的 ASCII 码，用 Python 命令行可以很方便地得到每个字符的 ASCII 码的二进制值。

>>bin(ord('h'))    # Python 命令，可直接看二进制
>>01101000
>>bin(ord('e'))  
>>01100101
>>bin(ord('l'))
...
![Snip20181118_16](https://ws4.sinaimg.cn/large/006tNbRwgy1fxcgml60x6j30qq0fwtbb.jpg)
...
上面这个例子可以理解为「零存整取」，注意是**取**，比如上面的`get s`。同样我们还也可以「零存零取」，「整存零取」。「零存」就是使用 setbit 对位值进行逐个设置，「整存」就是使用字符串一次性填充所有位数组，覆盖掉旧值。
...
比如我们可以通过 bitcount 统计用户一共签到了多少天，通过 bitpos 指令查找用户从哪一天开始第一次签到。如果指定了范围参数[start, end]，就可以统计在某个时间范围内用户签到了多少天，用户自某天以后的哪天开始签到。参考：https://blog.csdn.net/fly910905/article/details/82629687
遗憾的是， start 和 end 参数是**字节索引**(按字节为单位)，也就是说指定的位范围必须是 8 的倍数，而不能任意指定。这很奇怪，我表示不是很能理解 Antirez 为什么要这样设计。因为这个设计，我们无法直接计算某个月内用户签到了多少天，而必须要将这个月所覆盖的**字节**内容全部取出来 (getrange 可以取出字符串的子串) 然后在内存里进行统计，这个非常繁琐。
...
(integer) 1
127.0.0.1:6379> bitpos w 1 1 1  # 从第二个字符算起，第一个 1 位。最后两位数字是范围
(integer) 9
...
不过 Redis 的 3.2 版本以后新增了一个功能强大的指令，有了这条指令，不用管道也可以**一次进行多个位的操作**。
bitfield 有三个子指令，分别是 get/set/incrby，它们都可以对指定位片段进行读写，但是最多只能处理 64 个连续的位，如果超过 64 位，就得使用多个子指令，bitfield 可以一次执行多个子指令。

![Snip20181118_11](https://ws3.sinaimg.cn/large/006tNbRwgy1fxccl2jlqfj314q0mgabx.jpg)
...
(integer) 6       #0110，也就是10进制的6
127.0.0.1:6379> bitfield w get u3 2  # 从第三个位开始取 3 个位，结果是无符号数 (u)
(integer) 5       #101，也就是10进制的5
127.0.0.1:6379> bitfield w get i4 0  # 从第一个位开始取 4 个位，结果是有符号数 (i)
1) (integer) 6   #0110，也就是10进制的6
127.0.0.1:6379> bitfield w get i3 2  # 从第三个位开始取 3 个位，结果是有符号数 (i)
1) (integer) -3   #101，这里不知道为什么是-3
```
...
1) (integer) 101    ##返回旧值 e 对应二进制 01100101
...
1) (integer) 11        #1010对应十进制10，加1后，是1011，也就是十进制的11
