...
指一种有状态的锁，当锁没有达到“结束”状态时，会阻塞所有线程。当达到结束状态，闭锁唤醒线程，让所有线程继续执行。闭锁达到“结束”状态后，将不会再改变状态。闭锁可以用来确保某些活动在其他活动结束后再继续进行。



#### CountDownLatch

是闭锁的一种实现，它可以是一个或多个线程等在一组事件的发送。闭锁状态包括一个计数器，该计数器初始化为一个正数，表示需要等待的事件数量。countDown方法递减计数器，每次递减则表示有一个事件已经发生了。waait方法会阻塞当前线程，直到计数器达到0。（可能也会导致线程中断、超时）
...
FutureTask也可以用作闭锁，表示一种抽象的、可生成结果的计算。

Future.get的行为取决于任务的状态，如果任务已经完成了，则立即返回。否则将阻塞到任务完成为止，然后返回结果或者抛出异常。
